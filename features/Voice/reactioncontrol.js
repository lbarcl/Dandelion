...
const favs = require('../../utils/playlist/fav')
const serverList = require('../../utils/playlist/server')
  ...
  client.on('messageReactionAdd', async (reaction, user) => {
    ...
    switch (reaction.emoji.name) {
      ...
      case '🆑':
        ...
        break
      case '❤️':
        favs(reaction, server.queue.url[0], client, user)
        break
      case '🗒️':
        if (member.hasPermission('MENAGE_GUILD')){
          serverList(reaction, server.queue.url[0], client, reaction.message.guild)
        } else deleteAfterSend('Bu listeyi düzenlemek için yetkiniz yok')
        break
    }
  })
}